namespace WorkChronicle.ViewModels
{
    public partial class MainViewModel : BaseViewModel
    {
        ...
        public MainViewModel(ISchedule<IShift> schedule) //Using Dependency injection in the constructor for the DB repo as well.
        { 
        ...
        public async Task RefreshThePage()
        {
            if (this.Schedule.WorkSchedule.Count == 0 || this.Schedule == null)
            {
                try
                {
                    ...
                    // Check if the DB repo is not empty calls the method LoadFromTheDBSchedule
                    //and the method GenerateCalendarMonthYearAndTotalHours.
                    //if the DB Repo is empty call the method from the DB Repo GetClosestSchedule
        ...
        private async Task LoadFromDBToSchedule()
        {
            ...
            try
            {
                // Get the shifts for the DB repo for the required schedule
                // Add them in the schedule.WorkSchedule, that is bind with the UI.
        ...
        private async Task GenerateBlankCalendar()
        {
            try
            {
                ...
                // Calls the Main engine class from the Structured class library.
